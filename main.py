...
for command in commands:
    ...
    with apiServer.etcdLock:
        ...
        if cmdAttributes[0] == 'Deploy':
            ...
            # set up loadbalancer based on this dep
            loadbalancer = LoadBalancer(kind, apiServer, deployment)
            # start lb thread for this dep
        ...
        # remove dep and stop load balancer associated with dep
        ...
        # remove ms and it's hpa
            ...
            for hpa in hpas:
                if hpa.microserviceLabel == cmdAttributes[1]:
                    hpa.running = False
        # request in and push requests to deployment
        elif cmdAttributes[0] == 'ReqIn':
            apiServer.PushReq(cmdAttributes[1:])
        # create hps for each ms
        ...
        for dep in apiServer.etcd.deploymentList:
            if dep.deploymentLabel == graphedDep.deploymentLabel:
                ...
                for ms in dep.msList:
                    ...
                    pendingPods = list(filter(lambda x: x.microserviceLabel == ms,
                                              filter(lambda x: x.deploymentLabel == dep.deploymentLabel,
                                                     apiServer.etcd.pendingPodList)))
                    activePods = len(apiServer.GetEndPointsByLabel(dep.deploymentLabel, ms))
                    failedPods = list(filter(lambda x: x.microserviceLabel == ms,
                                             filter(lambda x: x.deploymentLabel == dep.deploymentLabel,
                                                    filter(lambda x: x.status == "FAILED",
                                                           apiServer.etcd.runningPodList))))
